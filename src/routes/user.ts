...
/* POST Signup user. */
router.post("/signup", validationMiddleware(User), async (req: express.Request, res: express.Response, next: express.NextFunction) => {
  try {
...
router.put("/reset-password", async (req: express.Request, res: express.Response, next: express.NextFunction) => {
  try {
    ...
    const data = await RestUserPasswordController(email, verificationCode, newPassword);
